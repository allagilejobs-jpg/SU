...
async function main() {
  ...
  fs.mkdirSync(path.join(__dirname, 'precise2'), { recursive: true });
  ...
  for (const seg of whisperData.segments) {
    ...
    // Add gap segment if there's a pause before this sentence
    const gapBefore = seg.start - lastEndTime;
    if (gapBefore > 0.05 && segNum > 0) {
      // Use the last segment's image for the gap
      segments.push({ file: `seg_${String(segNum-1).padStart(3, '0')}.png`, duration: gapBefore });
      console.log(`gap: ${gapBefore.toFixed(2)}s`);
    }
    
    const slide = getSlideForTime(seg.start);
    
    for (let wordIdx = 0; wordIdx < seg.words.length; wordIdx++) {
      const w = seg.words[wordIdx];
      const duration = w.end - w.start;
      
      const wordsHTML = generateWordHTML(seg.words, wordIdx);
      const html = generateHTML(slideContents[slide], wordsHTML);
      
      const htmlPath = path.join(__dirname, 'precise2', `seg_${String(segNum).padStart(3, '0')}.html`);
      const pngPath = path.join(__dirname, 'precise2', `seg_${String(segNum).padStart(3, '0')}.png`);
      ...
      console.log(`seg_${segNum}: "${w.word.trim()}" (${duration.toFixed(2)}s)`);
      segNum++;
  ...
  // Add final padding to reach ~52s if needed
  const totalDuration = segments.reduce((sum, s) => sum + s.duration, 0);
  console.log(`Total duration: ${totalDuration.toFixed(2)}s`);
  if (totalDuration < 51.5) {
    const padding = 51.7 - totalDuration;
    segments.push({ file: `seg_${String(segNum-1).padStart(3, '0')}.png`, duration: padding });
    console.log(`final padding: ${padding.toFixed(2)}s`);
  }
  ...
  segments.forEach(seg => {
    concatFile += `file 'precise2/${seg.file}'\n`;
    concatFile += `duration ${seg.duration.toFixed(4)}\n`;
  });
  concatFile += `file 'precise2/${segments[segments.length-1].file}'\n`;
  
  fs.writeFileSync(path.join(__dirname, 'concat-precise2.txt'), concatFile);
  console.log(`\nDone! Generated ${segNum} word images. Total segments with gaps: ${segments.length}`);
}
